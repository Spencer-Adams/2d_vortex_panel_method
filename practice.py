...
class vortex_panels:
    ...
    def pull_nodes(self, i):
        ...
        with open(foil, 'r') as text_handle:
            points = [list(map(float, line.strip().split())) for line in text_handle]  ## INPUT_geometry ##
        points_array = np.array(points)
        self.geometry = points_array
    ...
    def pull_alpha(self, j):
        with open(self.json_file, 'r') as json_handle:
            input_vals = json.load(json_handle)
            alpha = np.radians(input_vals["alpha[deg]"][j]) ## INPUT_alpha ##
        self.alpha = alpha
        
    def pull_vel_inf(self):
        with open(self.json_file, 'r') as json_handle:
            input_vals = json.load(json_handle)
            vel_inf = input_vals["freestream_velocity"]
    ...
    def calc_L(self):
        ...
        for j in range(len(points_list)-1):
            x1, y1 = points_list[j]
            x2, y2 = points_list[j+1]
            L_val = math.sqrt((x2-x1)**2 + (y2-y1)**2)
            L_list.append(L_val)
        L_group = np.array(L_list) # putting the brackets around l_j_list makes it a 1xN list rather than just saying np.array(l_j_list), which would output an Nx1 array.
        # print(L_group)
        self.L = L_group
    ...
    def calc_Cm_4(self):
        ...
        Cm_4 = self.Coeff_mle+ 0.25*self.Coeff_L
        self.Cm_4 = Cm_4
